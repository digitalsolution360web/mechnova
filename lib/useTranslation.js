...
export function TranslationProvider({ children }) {
  ...
  useEffect(() => {
    const detectLocale = async () => {
      try {
        ...
        if (savedLocale && (savedLocale === 'en' || savedLocale === 'es'|| savedLocale === 'hi')) {
  ...
  const switchLocale = () => {
  setLocale(prev => {
    if (prev === 'en') return 'es';   // English → Spanish
    if (prev === 'es') return 'hi';   // Spanish → Hindi
    return 'en';                      // Hindi → English
  });
};
